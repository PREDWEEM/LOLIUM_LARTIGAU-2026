...
# Actualización:
# - ADAPTACIÓN LARTIGAU: Coordenadas mantenidas estrictamente en -38.6166.
...
# ---------------------------------------------------------
def optimizar_parametros_hidricos_3d(df_meteo, df_campo, modelo_ann, latitud_lartigau=-38.6166, rango_ventanas=[7]):
    df = df_meteo.copy()
...
umbral_er = st.sidebar.slider("Umbral Tasa Diaria (Detección pico)", 0.001, 0.80, 0.001)
    ...
    if st.button("Ejecutar Barrido 3D"):
        if df_meteo_raw is not None and df_campo_raw is not None and modelo_ann is not None:
            ...
            with st.spinner(f'Ejecutando iteraciones 3D en ventanas: {ventanas_a_probar}...'):
                ...
                tabla_optima = optimizar_parametros_hidricos_3d(df_meteo_opt, df_campo_opt, modelo_ann, latitud_lartigau=-38.6166, rango_ventanas=ventanas_a_probar)
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # Balance Hídrico Superficial (Lartigau)
    ...
    # --- CÁLCULO DE FECHA DE CONTROL Y FECHA LÍMITE ---
    ...
    with tab1:
        ...
        with col_main:
            ...
            if fecha_control:
                ...
                if fecha_limite:
                    # Línea límite a los 800 °Cd (o lo que dictamine dga_critico)
    ...
    st.sidebar.download_button("📥 Descargar Reporte Completo", output.getvalue(), "PREDWEEM_Integral_Lartigau_vK4_9_15_UX.xlsx")
