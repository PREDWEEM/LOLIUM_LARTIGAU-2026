...
# - BYPASS HÍDRICO ELIMINADO: La lluvia modula la salida ANN, pero no fuerza la emergencia.
...
def optimizar_parametros_hidricos_2d(df_meteo, df_campo, modelo_ann, latitud_lartigau=-38.6166):
    ...
    df["ET0"] = calcular_et0_hargreaves(df["Julian_days"].values, df["TMAX"].values, df["TMIN"].values, latitud=latitud_lartigau)
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # 2. Balance Hídrico Superficial (Lartigau)
    # La lluvia modula la salida de la red, pero ya no fuerza EMERREL.
    ...
    # 3. Escudo Termofisiológico — HORIZONTE DE TERMOINHIBICIÓN ADAPTADO A 5 DÍAS
    ...
    # 4. BLOQUEO FINAL ESTRICTO: Latencia Temprana (Primeros 45 días del año)
