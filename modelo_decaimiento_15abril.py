# -*- coding: utf-8 -*-
"""Decaimiento tardío de PREDWEEM Lartigau desde el 15 de abril.

Replica el criterio operativo aplicado en Bordenave:
...
def _reemplazar_unico(source: str, old: str, new: str, etiqueta: str) -> str:
    ...
    if cantidad != 1:
        raise RuntimeError(
            f"Parche de decaimiento Lartigau no aplicado: '{etiqueta}' aparece "
...
def parchear_modelo_decaimiento_15abril(source: str) -> str:
    """Inserta techo 50 % + decaimiento desde 15-abr dentro de simulate_emergence."""

    motor_old = '''    data, first_peak_index = apply_first_peak_filter(
        data,
    ...
    motor_new = '''    data, first_peak_index = apply_first_peak_filter(
        data,
        threshold=UMBRAL_PRIMER_PICO,
    )
