# -*- coding: utf-8 -*-
# ===============================================================
# 🌾 PREDWEEM INTEGRAL vK4.9.15 — LOLIUM BUENOS AIRES 2026
# Actualización y Rigor Científico:
# - ADAPTACIÓN BUENOS AIRES: Coordenadas fijas en -34.6037 para ET0 y balances.
# - IDENTIDAD: PREDWEEM by GUILLERMO R. CHANTRE.
# - LATENCIA INICIAL: Bloqueo estricto de emergencia los primeros 25 días del año.
# - CHOQUE HÍDRICO CONSERVADO: Umbral de lluvia acumulada de 3 días = 15 mm.
...
if 'arranque_fase' not in st.session_state:
    st.set_page_config(page_title="PREDWEEM BUENOS AIRES INTEGRAL", layout="wide", page_icon="🌾")
    st.markdown("<br><br><br>", unsafe_allow_html=True)
    st.info("🚜 **Iniciando Servidor PREDWEEM Buenos Aires...** Cargando motores dinámicos por evento.")
    st.progress(20)
...
def calcular_et0_hargreaves(jday, tmax, tmin, latitud=-34.6037):
...
def aplicar_filtro_primer_pico(df, umbral=UMBRAL_PRIMER_PICO):
    """
    Habilita la campaña a partir del primer valor de EMERREL
    estrictamente superior al umbral. Los flujos anteriores se
    conservan en una columna de auditoría y se cancelan en EMERREL.
    """
    df = df.copy()
...
def optimizar_parametros_hidricos_2d(
    ...
    df["ET0"] = calcular_et0_hargreaves(df["Julian_days"].values, df["TMAX"].values, df["TMIN"].values, latitud=latitud_caba)
    ...
    for w_max in rango_w_max:
        for ke in rango_ke:
            ...
            # Choque hídrico conservado: lluvia acumulada de 3 días
            ...
            # Misma condición de inicio usada por el motor principal
...
st.title("🌾 PREDWEEM LOLIUM — CIUDAD DE BUENOS AIRES lat=-34.6037 lon=-58.3816")

with st.expander("📂 1. Datos del Lote", expanded=True):
    col_upload, col_rastrojo = st.columns(2)
    
    with col_upload:
        archivo_meteo = st.file_uploader("1. Clima (Buenos Aires)", type=["xlsx", "csv"])
    ...
    with col_rastrojo:
        with st.container(border=True):
            ...
            cobertura_pct = st.slider(
                ...
                help="0% = Suelo desnudo. 100% = Cobertura total."
            )

            x_cobertura = [0, 30, 70, 100]
            ke_val = float(np.interp(cobertura_pct, x_cobertura, [0.80, 0.50, 0.25, 0.10]))
...
df_campo_raw = load_data(archivo_campo, "CABA_campo")
...
with st.sidebar.expander("🛠️ Modo Dev: Calibrador Bio-Físico 2D", expanded=False):
    st.caption("Mapea combinaciones de Capacidad de Campo y Evaporación contra ventanas de muestreo reales.")
    if st.button("Ejecutar Barrido Hídrico"):
        if df_meteo_raw is not None and df_campo_raw is not None and modelo_ann is not None:
            with st.spinner('Ejecutando iteraciones hídricas puras...'):
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # 2. Choque Hídrico de Ruptura Temprana
    # Se conserva la lógica original; el umbral predeterminado es 15 mm.
    ...
    # 3. Balance Hídrico Superficial (Buenos Aires)
    df["ET0"] = calcular_et0_hargreaves(df["Julian_days"].values, df["TMAX"].values, df["TMIN"].values, latitud=-34.6037)
    ...
    with tab2:
        st.header("💧 Dinámica Hídrica del Suelo (Buenos Aires)")
    ...
    with tab3:
        st.header("🔍 Clasificación DTW (Buenos Aires)")
    ...
    st.sidebar.download_button("📥 Descargar Reporte Buenos Aires", output.getvalue(), "PREDWEEM_Integral_Buenos_Aires_vK4_9_15.xlsx")

else:
    st.info("👋 Bienvenido a PREDWEEM. Cargue los datos climáticos de Buenos Aires para comenzar.")
